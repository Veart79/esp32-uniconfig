...
mode - Хранит режим работы. Напр. 0 - автоматический 1 - ручной. Сохраняется после перезагрузки.
Значение переменной можно устанавливать в действиях:
```
"actions": {
  "mode1": {"var": "mode", "value": 1}
}
```

init - принимает значение 1 при запуске (в первом цикле проверки правил). Далее = 0.
Использовать в правилах для инициализации состояния контроллера
```
{"exp": "init", "actions": ["disableLed", "enablePump"]}
```
Переменную можно использовать в более сложных условиях типа ``` "exp": "init=1 & mode=1 & t>40" ```  Напр. чтобы попытаться определить реальный первый запуск контроллера от перезагрузки при скачке напряжения.
